...
const Merge = () => {
  ...
  const handleStartMerge = async () => {
    ...
    // In a real implementation, you would use FFmpeg.wasm or a backend API
  ...
  const handleDownload = () => {
    ...
    alert('下载功能需要后端支持或 FFmpeg.wasm 集成');
  };
  ...
  return (
    <div className="flex-1 relative flex flex-col items-center p-4 md:p-8 overflow-y-auto bg-background-light dark:bg-background-dark">
      <div className="w-full max-w-3xl flex flex-col gap-8 my-auto">
        ...
        {state.videoFile && state.audioFile && state.status === 'idle' && (
        ...
        {state.status === 'processing' && (
          <div className="w-full bg-white dark:bg-[#1a1a1a] border border-border-light dark:border-border-dark rounded-2xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <p className="text-sm font-bold text-text-light dark:text-text-dark">合并中...</p>
        ...
        {state.status === 'error' && state.errorMessage && (
          <div className="w-full p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-2xl">
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-2xl flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-blue-600 dark:text-blue-400 leading-relaxed">
                此工具可以将视频文件和音频文件合并为一个新的视频文件。适用于为无声视频添加背景音乐，或替换视频原有音轨等场景。
              </p>
            </div>
          </div>
        </div>
